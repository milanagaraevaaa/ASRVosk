...
######################################
# 2. Преобразование аудио для Whisper
######################################
...
def save_waveform_to_temp_file(waveform, temp_path):
    ...
    torchaudio.save(temp_path, waveform.unsqueeze(0), sample_rate=16000)

######################################
# 3. Функция транскрибирования с Whisper
######################################
def transcribe_with_whisper(audio_path, model, language="ru"):
    ...
    print(f"Аудио сохранено во временный файл: {temp_wav_path}")

    try:
        result = model.transcribe(temp_wav_path, language=language, fp16=False)
...
def compute_metrics(reference, hypothesis):
    ...
    # Убедимся, что тексты токенизированы корректно
    rouge_scores = scorer.score(" ".join(ref_tokens), " ".join(hyp_tokens) if hyp_tokens else "")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        transcription = transcribe_with_whisper(audio_path, model)
    ...
    for idx in indices:
        ...
        transcription = transcribe_with_whisper(waveform, model)
